...
const TopSellersChart = () => {
    ...
    // Retrieve top selling products from api
    useEffect(() => {
    ...
    useEffect(() => {
        const getNames = async () => {
            // Retrieve ids from items
            try {
                const ids = topData.map((item) => {
                    return item._id;
                })
                // Find all products with ids corresponding to ids in topData
                const res = await userReq.get(`/products/findall/${ids}`);
                const tempData = topData;
                // Map through and append title to items in tempData
                res.data.map((item) => {
